...
Если результат работы селектора – простой тип данных, то результат сравнения при равных значениях будет отрицательным, и ререндер компонента не случится.
...
В таких случаях прибегают к использованию дополнительной функции `shallowEqual`, которая, в случае объектов сравнивает значения полей. и в случае разницы об этом сообщает.
...
// или отправляется как поле `equalityFn` внутри второго аргумента
...
Чтобы такого не было, используют другой хук – `createSelector()` –, который позволяет запомнить один раз какой-то селектора и пользоваться одним инстансом во многих местах.
